...
3. **must_visit 必须出现** — 用户在 `intent.must_visit` 列出的地名（"必去 X / 一定要去 X"）必须出现在 stops 中。这些 POI 已经在候选池 city_essential 头部（manual_priority=999）。如果 must_visit 列表非空，输出 stops 中**至少要命中** must_visit 列表的每一项（按 POI 名字子串匹配或别名匹配，比如"故宫" ≈ "紫禁城博物院"）。漏掉就是输出无效。
4. **avoid 不能出现**。
...
`stops` 内容应满足:
- **≥ 2 种 categories** — 4 stop 不能全是同一大类 (例: 不能 4 stop 全 "美食"). 至少跨 2 个大类 (景点 / 美食 / 购物 / 休闲 任意 ≥ 2 种).
- **相邻 stop 类型不重复** — 上一站和下一站避免相同 categories[0]. (例: 不能上午景点 = 景点, 下午 = 景点; 中间应插非景点 stop).
- **同 city_zone 同 categories[0] 至多 2 个 stop** — 防止"4 个 stop 全在万象天地都是美食".

如果候选池本身只有一种类目 (例 layover_eat 全美食池), 按池实际分布走即可, 不强求多样.
