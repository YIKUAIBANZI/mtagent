...
from agents.poi_decision_signals import (
    get_decision_signals,
    prefer_pois_by_decision_signals,
    with_stop_decision_signals,
)
...
def _apply_decision_signals_to_stops(
    stops: list[Stop], intent: Optional[ParsedIntent]
) -> list[Stop]:
    """Attach precomputed UGC decision signals after scheduling."""
    return [with_stop_decision_signals(stop, intent) for stop in stops]
...
_CITY_ANCHORS: dict[str, list[tuple[str, float, float]]] = {
    ...
    "北京": [
        ("故宫", 39.9163, 116.3972),
        ("天安门", 39.9032, 116.3977),
        ("南锣鼓巷", 39.9338, 116.4036),
        ("三里屯", 39.9331, 116.4551),
        ("天坛", 39.8822, 116.4065),
    ],
    "南昌": [
        ("滕王阁", 28.6813, 115.8812),
        ("万寿宫", 28.6741, 115.8880),
        ("八一广场", 28.6739, 115.9045),
        ("秋水广场", 28.6831, 115.8627),
        ("江西省博物馆", 28.7059, 115.8818),
    ],
}
...
class Planner:
    ...
    async def run(self, ctx: TripContext) -> RouteDraft:
        ...
        for d, day_data in enumerate(llm_data.get("days", [])):
            ...
            stops = _apply_decision_signals_to_stops(stops, intent)
            days_out.append(
                DayPlan(
    ...
    async def compose_one_day(
        ...
        _synthesize_fallback_route + compute_day_transits.
        """
        ...
        poi_brief = [
            {
                ...
                "decision_signals": get_decision_signals(p.openshopid),
            }
        ...
        return json.dumps(
            {
                ...
                "_instruction": (
                    ...
                    "若 candidate 带 decision_signals，必须参考其中的 queue_risk / best_time / reservation：用户怕排队时避开 high queue 的正餐高峰，保留时要错峰或给预约缓冲。"
...
def _pick_near_centroid(
    ...
    if not stops_so_far:
        matches = [
            p
            for p in cluster
            if p.openshopid not in used
            and any(c in slot.category_pool for c in p.categories)
        ]
        matches = prefer_pois_by_decision_signals(
            matches, intent=intent, slot_name=slot.name, variant=variant
        )
        if matches:
            return matches[0]
        return None
