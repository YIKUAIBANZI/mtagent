...
def test_adjust_replace_stop_with_target_poi_id_prefers_exact_candidate(
    sse_app_client,
):
    old = _mk_poi("OLD", "旧午餐", role="meal", zone="A")
    fallback = _mk_poi("FALLBACK", "普通替代", role="meal", zone="A")
    target = _mk_poi("TARGET", "用户点选餐厅", role="meal", zone="A")
    ...
    ctx.draft_route = RouteDraft(
        days=[
            DayPlan(
                day_index=0,
                anchor_district="A",
                stops=[_mk_stop(old, "午饭", 12, 13)],
            )
        ]
    )
    ctx.candidate_pois = [fallback, target]
    ctx.save()

    resp = sse_app_client.post(
        f"/api/plan/{ctx.trip_id}/adjust",
        json={
    ...
    assert resp.status_code == 200
    events = _parse_events(resp.text)
    replaced = next(e for e in events if e["event"] == "adjust.stop_replaced")
    assert replaced["data"]["source"] == "target"
    assert replaced["data"]["new_stop"]["poi"]["openshopid"] == "TARGET"
